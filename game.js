...
function spawnMissile(attacker, target, now) {
...
function updateExplosions(now) {
  ...
  for (const explosion of state.explosions) {
    ...
    if (now - explosion.createdAt >= duration) {
      ...
      if (explosion.owner.mode === "ultimate") {
        explosion.owner.mode = "normal";

        if (explosion.owner.isPlayer) {
...
function updateUltimate(fighter, now) {
  ...
  if (fighter.type === "soldier") {
    const launchAt = CONFIG.ULTIMATE_FRAME_MS * 5;

    // ultimate06 프레임에서 미사일을 한 번 생성합니다.
    if (
      !fighter.ultimateProjectileSpawned &&
      elapsed >= launchAt
    ) {
      ...
      spawnMissile(fighter, target, now);
      fighter.ultimateProjectileSpawned = true;
    }

    // 폭발 종료 시 updateExplosions()에서 Idle로 복귀합니다.
    return;
  }
