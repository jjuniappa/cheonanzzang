...
// 오른쪽을 향하도록 제작된 수리검 에셋을 실제 발사 방향에 맞게 회전해서 그립니다.
function drawDirectionalSprite(image, centerX, centerY, size, directionX, directionY) {
  const scale = Math.min(size / image.width, size / image.height);
  const width = image.width * scale;
  const height = image.height * scale;
  ...
  for (const projectile of state.projectiles) {
    ...
    if (image) {
      drawDirectionalSprite(
        ...
        p.x,
        p.y,
        CONFIG.SHURIKEN_SIZE,
        projectile.directionX,
        projectile.directionY
      );
    } else {
      // 테스트용 도형도 발사 방향에 맞춰 회전합니다.
